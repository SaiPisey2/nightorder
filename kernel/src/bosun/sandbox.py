...
def build_sandbox_job_manifest(*, name: str, image: str, command: list[str],
    ...
    return {
        ...
        "metadata": {
            "name": name,
            "namespace": namespace,
            "labels": {"app.kubernetes.io/managed-by": "bosun",
                       "bosun/component": "sandbox", **(labels or {})},
        },
        "spec": {
            ...
            "template": {
                "metadata": {"labels": {"bosun/component": "sandbox"}},
                "spec": {
                    "restartPolicy": "Never",
                    "serviceAccountName": "bosun-sandbox",
...
def _job_name(idempotency_key: str) -> str:
    return "bosun-sbx-" + hashlib.sha256(idempotency_key.encode()).hexdigest()[:12]
...
def _run_sync(manifest: dict, namespace: str, timeout_seconds: int) -> dict:
    ...
    try:
        ...
        logs += f"\n(log fetch failed: {e})"
    from bosun.builtins.executors import redact
