...
from bosun.contracts.base import GateChannel, GateNotification

log = logging.getLogger("bosun.gates")
...
class TeamsChannel(GateChannel):
    ...
    channel_params:
      url: webhook URL (default: BOSUN_TEAMS_WEBHOOK_URL)
    ...
    async def deliver(self, notification: GateNotification, params: dict[str, Any]) -> None:
        ...
        if not url:
            raise ValueError("teams channel: no webhook url (channel_params.url or BOSUN_TEAMS_WEBHOOK_URL)")
