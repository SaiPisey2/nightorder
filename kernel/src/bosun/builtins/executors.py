...
from bosun.contracts.base import ExecutionResult, StepContext, StepExecutor
...
class ArgoWorkflowExecutor(StepExecutor):
    ...
    config:
      ...
      manifest_file: path relative to BOSUN_MANIFESTS_DIR — the worker loads
    ...
    Idempotency: workflow name = bosun-<sha of idempotency key>. A 409 on
    ...
    def _execute_sync(self, ctx: StepContext, config: dict[str, Any]) -> ExecutionResult:
            ...
            if not os.path.isfile(path):
                return ExecutionResult(status="failed",
                                       logs=f"argo executor: manifest_file not found: {path} "
                                            f"(BOSUN_MANIFESTS_DIR={cfg.manifests_dir})")
            with open(path) as f:
        ...
        manifest["metadata"].setdefault("labels", {}).update(
            {
                "bosun/project": ctx.project,
                "bosun/run-id": ctx.run_id[:63],
                "bosun/step-id": ctx.step_id[:63],
...
class RemoteExecExecutor(StepExecutor):
    ...
    async def execute(self, ctx: StepContext, config: dict[str, Any]) -> ExecutionResult:
        ...
        sentinel = f"/tmp/bosun-{key_digest}.done"
        guarded = f"if [ -e {sentinel} ]; then echo BOSUN_ALREADY_RAN; else ({command}) && touch {sentinel}; fi"
        try:
