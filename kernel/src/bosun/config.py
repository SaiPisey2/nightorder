...
class Settings:
    database_url: str = field(
        default_factory=lambda: _env(
            "BOSUN_DATABASE_URL",
            "postgresql+asyncpg://bosun:bosun@localhost:5433/bosun",
        )
    )
    ...
    api_base_url: str = field(default_factory=lambda: _env("BOSUN_API_BASE_URL", "http://localhost:8400"))
    # HMAC secret for signed single-use gate action tokens.
    gate_token_secret: str = field(
        default_factory=lambda: _env("BOSUN_GATE_TOKEN_SECRET", "dev-only-secret-change-me")
    )
    gate_token_ttl_seconds: int = field(
        default_factory=lambda: int(_env("BOSUN_GATE_TOKEN_TTL_SECONDS", "86400"))
    )
    # Anthropic (advisor/agents). Key comes from env only — never persisted.
    anthropic_model: str = field(default_factory=lambda: _env("BOSUN_ADVISOR_MODEL", "claude-sonnet-5"))
    # Qdrant semantic memory (Phase 2). Retrieval only — never workflow state.
    qdrant_url: str = field(default_factory=lambda: _env("BOSUN_QDRANT_URL", "http://localhost:6333"))
    # Sandbox execution (Phase 3)
    sandbox_namespace: str = field(default_factory=lambda: _env("BOSUN_SANDBOX_NAMESPACE", "bosun-e2e"))
    # Teams (Power Automate webhook). Default channel for gates + relay alerts.
    teams_webhook_url: str = field(default_factory=lambda: _env("BOSUN_TEAMS_WEBHOOK_URL", ""))
    # Which events the relay forwards to Teams as alerts (csv).
    teams_alert_events: str = field(
        default_factory=lambda: _env(
            "BOSUN_TEAMS_ALERT_EVENTS",
            "WorkflowFailed,GateRequested,RemediationSuggested,GateExpired",
        )
    )
    # Emails to @mention on relay alerts (csv).
    teams_alert_mentions: str = field(default_factory=lambda: _env("BOSUN_TEAMS_ALERT_MENTIONS", ""))
    # Kubernetes context for the Argo executor ("" = current context / in-cluster).
    kube_context: str = field(default_factory=lambda: _env("BOSUN_KUBE_CONTEXT", ""))
    argo_namespace: str = field(default_factory=lambda: _env("BOSUN_ARGO_NAMESPACE", "argo"))
    ...
    # execution time.
    manifests_dir: str = field(default_factory=lambda: _env("BOSUN_MANIFESTS_DIR", "manifests"))
