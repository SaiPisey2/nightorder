...
Run as its own process: `nightorder-relay`.
"""
...
from nightorder.db import Event, KnowledgeRecord, Outbox, get_session, init_db

log = logging.getLogger("nightorder.relay")
...
async def _step_notification(event: Event) -> bool:
    ...
    message = build_adaptive_card(
        ...
        buttons=[{"title": "Open Nightorder", "url": cfg.api_base_url.rstrip("/") + "/ui/"}],
...
async def _teams_alert(event: Event) -> None:
    ...
    message = build_adaptive_card(
        ...
        facts=facts,
        buttons=[{"title": "Open Nightorder", "url": ui}],
        mentions=mentions,
        color=color,
    )
    await post_to_teams(webhook_url, message)
    ...
    for outbox_row, event in rows:
        try:
            ...
            async with get_session() as session:
                ...
                row.published = True
                from nightorder.db.models import utcnow
