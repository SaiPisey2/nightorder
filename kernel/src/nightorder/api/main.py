...
log = logging.getLogger("nightorder.api")

app = FastAPI(title="Nightorder", version=__version__)
...
async def health_components() -> dict:
    ...
    from nightorder.db import Outbox
    ...
    async def check_qdrant() -> dict:
        try:
            from nightorder.ai.knowledge import qdrant_available
...
async def list_knowledge(project: str, limit: int = 100, _: Project = Depends(require_project)) -> list[dict]:
    from nightorder.db import KnowledgeRecord
...
async def start_run(project: str, pipeline: str, body: RunCreate, _: Project = Depends(require_project)) -> dict:
    async with get_session() as session:
        ...
        await session.flush()
        run_id = run.id
        workflow_id = f"nightorder-run-{run_id}"
        run.temporal_workflow_id = workflow_id
        await session.commit()

    from nightorder.temporal.workflows import PipelineRunWorkflow, RunInput
    ...
    async with get_session() as session:
        ...
        if gate and gate.gate_type == "remediation_approval":
            ...
            if incident:
                ...
                if decision == "approve":
                    from nightorder.temporal.activities import _emit as emit_event
                    from nightorder import events as ev
...
async def create_schedule(project: str, pipeline: str, _: Project = Depends(require_project)) -> dict:
    ...
    schedule_id = f"nightorder-sched-{project}-{pipeline}"
    client = await temporal_client()
    ...
    await client.create_schedule(
        ...
        Schedule(
            action=ScheduleActionStartWorkflow(
                ...
                id=f"nightorder-sched-run-{project}-{pipeline}",
                task_queue=TASK_QUEUE,
            ),
...
async def delete_schedule(project: str, pipeline: str, _: Project = Depends(require_project)) -> dict:
    client = await temporal_client()
    handle = client.get_schedule_handle(f"nightorder-sched-{project}-{pipeline}")
    await handle.delete()
    return {"deleted": True}
...
async def troubleshoot_step(run_id: str, step_id: str, body: TroubleshootRequest | None = None,
    ...
    from nightorder.ai.troubleshoot import run_troubleshooting
...
async def report_outcome(incident_id: str, body: OutcomeReport, x_api_key: str = Header(default="")) -> dict:
    ...
    from nightorder.db import Incident, KnowledgeRecord
    from nightorder.temporal.activities import _emit as emit_event
    from nightorder import events as ev
...
async def register_playbook(project: str, body: PlaybookCreate, _: Project = Depends(require_project)) -> dict:
    from nightorder.db import Playbook
...
async def execute_remediation(incident_id: str, body: ExecuteRemediation,
    ...
    from nightorder.db import Incident, Playbook
    ...
    from nightorder.temporal.workflows import RemediationWorkflow, RemediationInput
    from nightorder import TASK_QUEUE
    ...
    handle = await client.start_workflow(
        ...
        id=f"nightorder-remediation-{incident_id}",
        task_queue=TASK_QUEUE,
    )
...
async def project_chat(project: str, body: ChatRequest, _: Project = Depends(require_project)) -> dict:
    ...
    from nightorder.ai.chat import chat as run_chat
...
async def diagnose_step(run_id: str, step_id: str, run: Run = Depends(project_for_run)) -> dict:
    from nightorder.advisor.diagnose import advisor_available, diagnose
