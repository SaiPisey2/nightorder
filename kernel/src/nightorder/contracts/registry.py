...
Teams register extensions in their package's pyproject.toml:
    ...
    [project.entry-points."nightorder.step_executors"]
    my_backend = "my_pkg.executors:MyBackendExecutor"

    [project.entry-points."nightorder.preflight_checks"]
...
from nightorder.contracts.base import GateChannel, ParameterResolver, PreflightCheck, StepExecutor

log = logging.getLogger(__name__)

GROUPS = {
    "step_executors": ("nightorder.step_executors", StepExecutor),
    "preflight_checks": ("nightorder.preflight_checks", PreflightCheck),
    "param_resolvers": ("nightorder.param_resolvers", ParameterResolver),
    "gate_channels": ("nightorder.gate_channels", GateChannel),
}
