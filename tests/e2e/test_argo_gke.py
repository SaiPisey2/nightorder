"""Argo executor against a real cluster. OPT-IN: set BOSUN_E2E_ARGO=1.

Submits one tiny, clearly-labeled alpine workflow (10m CPU / 32Mi) to the
namespace in BOSUN_ARGO_NAMESPACE using the kube context in
BOSUN_KUBE_CONTEXT. The workflow self-cleans via ttlStrategy; the test also
...
pytestmark = pytest.mark.skipif(
    os.environ.get("BOSUN_E2E_ARGO") != "1",
    reason="cluster test is opt-in: BOSUN_E2E_ARGO=1",
)
...
def test_argo_smoke_on_cluster(client, project, stack):
    ...
    assert step["external_ref"].startswith("bosun-e2e/bosun-"), step
